...
class stein_kernel():
    """
    Stein kernel built from:
      - target distribution π (provided via `distribution`, must implement .score(x))
      - base kernel: RBF with bandwidth sigma
    If `distribution` is None, falls back to N(0, std^2 I) score: -x/std^2
    """
    is_stein = True
    def __init__(self, sigma, distribution=None, std=1.0):
        self.sigma = sigma
    ...
    def _score(self, x):
        # Prefer user-provided distribution score
        if self.distribution is None:
            raise ValueError("stein_kernel requires a distribution with .score(x).")
        return self.distribution.score(x)

    def __call__(self, x, y):
        # # shape checks
        # if x.ndim != 1 or y.ndim != 1:
        #     raise ValueError(f"stein_kernel expects x,y to be 1D vectors, got {x.shape}, {y.shape}")
        # if x.shape[-1] != y.shape[-1]:
        #     raise ValueError(f"x and y must have same dim, got {x.shape[-1]} vs {y.shape[-1]}")

        dim = x.shape[-1]

        dvec = x - y
        r2 = jnp.sum(dvec * dvec)

        inv_sigma2 = 1.0 / (self.sigma * self.sigma)
        inv_sigma4 = inv_sigma2 * inv_sigma2

        # RBF kernel k(x,y) = exp( - ||x-y||^2 / (2 sigma^2) )
        kxy = jnp.exp(-0.5 * r2 * inv_sigma2)

        # ∇_x k, ∇_y k
        grad1 = -(dvec * inv_sigma2) * kxy
        grad2 = +(dvec * inv_sigma2) * kxy

        # ∇_x · ∇_y k  (general dim)
        # = (dim/sigma^2 - ||x-y||^2/sigma^4) k
        div12 = (dim * inv_sigma2 - r2 * inv_sigma4) * kxy
